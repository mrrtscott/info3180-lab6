...
const NewsList = Vue.component('news-list', {
    template: `<div class="news">
      <center><h2>News</h2></center>
          ...
          <ul class="news__list">
            <li v-for="article in articles" class="news__item"><h4>{{ article.title }}</h4> <img :src="article.urlToImage"/> <br> <br><p>{{article.description}} </p>
            </li>
          </ul>
      </div>
  ...
  `,
    created: function() {
        let self = this;
        fetch('https://newsapi.org/v2/top-headlines?country=us&apiKey=af050bba57be4cef8fdb27dcb3ba8126')
    ...
    methods: {
        searchNews: function() {
            let self = this;
            fetch('https://newsapi.org/v2/everything?q=' + self.searchTerm + '&language=en&apiKey=af050bba57be4cef8fdb27dcb3ba8126')
                .then(function(response) {
                    return response.json();
                })
                .then(function(data) {
